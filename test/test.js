...
describe("A transaction with no value", () => {
  let contract;
  let beneficiary;
  let arbiter;

  before("Create a wrong contract", async () => {
  ...
  it("should revert", async function () {
    await expect(
      contract.createContract(arbiter.getAddress(), beneficiary.getAddress(), {
        value: ethers.utils.parseEther("0"),
      })
    ).to.be.reverted;
  });
  it("should return no transactions saved", async function () {
    const txs = await contract.getListContracts();
